import {ArrowRight,BookmarkSimple,CaretRight,CheckCircle,Clock,Plus} from '@phosphor-icons/react';
...
export default function Home({games,activeGame,templateCount,onNew,onOpen,onHistory,onUnsettled,onTemplates,onSample}:{
  games:Game[];activeGame:Game|null;templateCount:number;
  onNew:()=>void;onOpen:(game:Game)=>void;onHistory:()=>void;onUnsettled:()=>void;onTemplates:()=>void;onSample:()=>void;
}){
  ...
  return <div className="home-page">
    ...
    <button className="button primary full home-primary" onClick={()=>activeGame?onOpen(activeGame):onNew()}>
      {activeGame?<><ArrowRight size={18}/>Resume game</>:<><Plus size={18}/>{ended.length?'New game':'Start a game'}</>}
    </button>
    {featured&&step&&<section className="home-last-game" aria-label={activeGame?'Current game':'Last game'}>
      ...
      {!activeGame&&<button className="home-card-action" onClick={()=>onOpen(featured)}>{step.action}<ArrowRight size={17}/></button>}
    </section>}
