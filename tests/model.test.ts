...
test('allows closing complete balanced games, including empty or single-player tables',()=>{const g=game();assert.ok(canEnd(g));g.players[0].cashout=null;assert.ok(!canEnd(g));g.players[0].cashout=0;assert.ok(!canEnd(g));g.players=[];assert.ok(canEnd(g));const one={...g,players:[{id:'a',name:'Alex',buyins:[5000],cashout:5000}]};assert.ok(canEnd(one));one.players[0].cashout=4000;assert.ok(!canEnd(one));});
