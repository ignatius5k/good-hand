...
2. Add players. Use the player card's Rebuy or Cash out action. Cash-out records the remaining chip value; it does not move money.
...
4. All players must cash out and total cash-outs must equal total buy-ins before ending the game. No rake or fees are deducted.
...
Amounts use integer cents. Zero chip cash-outs are supported. A capped undo history covers player changes during the current session. Closed games are immutable apart from payment status. Export JSON backups in Settings. Restoring merges games by ID without replacing existing games and rejects multiple open games. Existing IDs are intentionally kept, so restore does not overwrite a newer version of a game.
...
Typography: DM Sans and Instrument Serif, bundled locally. Icons: Phosphor. React, TypeScript, Vite and vite-plugin-pwa. Native HTML dialogs provide focus management, Escape dismissal and phone bottom sheets. An optional, feature-detected WebMCP `get_game_summary` tool exposes a read-only game summary; its contract is tested with an injected registry because the test browser lacks the proposed native API.
